---
title: "Section 2: Characterize the association between a set of iron-related predictors and baseline iron status for women who are premenopausal at baseline."
output:
  pdf_document:
    includes:
...
#labels.df$labels = gsub("bowl", "bowel", as.character(labels.df$labels)) # fix labels for correct spelling
```
...
#    {
      ...
      if(outcome=="Serum.ferritin") {
        fmla = paste( "scale(log(y))", '~ x + baseline.age')
      } else {
        fmla = paste( "scale(y)", '~ x + baseline.age')
        }
...
head(dat.cat2[dat.cat2$labels %in% c("rs855791, G allele"),])
...
ggsave("all-vars-ms-pre.png", units="in", width=15, height=14, dpi=400) #units="in", width=5, height=4, dpi=300,
