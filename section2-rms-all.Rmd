---
title: "Section 2: Characterize the association between a set of iron-related predictors and baseline iron status for all women combined (pre and postmenopausal) (using rms package)"
output:
  html_document:
    theme: united
    toc: no
...
# Section 2 RMS, postmenopause # {#s2postrms}
...
knitr::opts_chunk$set(echo = F,
                      eval=F, # set to eval=F if the different predictor modeling is done and just need to reformat tables
                      warning = F,
                      fig.pos = 'H',
                      float=F,
                      fig.keep=F,
...
dim(s2.dat)
s2.dat = s2.dat[s2.dat$menop.status %in% c(0,1),] # select women who are postmenopausal at baseline
dim(s2.dat)
table(s2.dat$menop.status2)
...
# convert menop status to binary numeric variable
s2.dat$menop.status2 = as.numeric(s2.dat$menop.status)
...
#sum(table(s2.dat$menop.status, useNA = "always"))
...
g3 = paste0( paste0("rcs(", repro.covs[!(repro.covs %in% c("menop.status2"))], ",3)", collapse = " + " ), " + menop.status2"); g3
...
fmla.iron.simple = as.formula(paste("scale(Serum.iron) ~ ", 
                              paste0( c(g1.simple, g2.simple, g3.simple,
                                        g4, g5, "scale(baseline.age, scale=F)",
                                        "menop.status2"),
                                      collapse = " + ")))
# run models
...
summary(lm(fmla.iron, data=s2.dat))$r.squared
...
v.iron.out = v.iron[c("R-square", "Intercept", "Slope"), c("index.orig", "optimism", "index.corrected")]
v.iron.out
...
m0.ferritin.simple$stats["R2"]
m0.ferritin$stats["R2"]
...
# load("W:/projects/iron-aim4/sections/t2-tabledat-pre.RData") # debug
load("t2-tabledat-all.RData")
...
labels.df = rbind.data.frame(ldf, 
                             ...
                             data.frame(labels = "Postmenopause status (y/n)", varname="menop.status2", category = "Menopause and HRT"))
labels.df
...
dat.coef.merge.iron = dat.coef.merge2[dat.coef.merge2$outcome == "Iron", vars.select]
...
save(v.out, file="s2-rms-all.RData")
...
save(dat.coef.merge.iron2, 
     ...
     dat.coef.mi.merge3, 
     boot.set, impute.set,
     file="s2-model-perf-tables-all.RData")
