---
title: "Section 2: Characterize the association between a set of iron-related predictors and baseline iron status for women who were postmenopausal at study entry. (using rms package)"
output:
  html_document:
    theme: united
    toc: no
...
dim(s2.dat)
s2.dat = s2.dat[s2.dat$menop.status==1,] # select women who are postmenopausal at baseline
dim(s2.dat)
...
fmla.iron <- as.formula(paste("scale(Serum.iron) ~ ", 
                              paste0( c(g1, g2, g3, g4, g5, "scale(baseline.age, scale=F)"),
                                      collapse = " + ")))
                ...
                ifelse(varname %in% health.vars, "Health conditions (ever)",
                       ifelse(varname %in% repro.health.vars, "Reproductive health", 
                              ifelse(varname %in% lifestyle.vars, "Lifestyle", 
                                     ifelse(varname %in% supp.vars, "Supplement and Diet", varname )))))
})
...
save(v.out, file="s2-rms-post.RData")
...
save(dat.coef.merge.iron2, 
     ...
     dat.coef.mi.merge3, 
     boot.set, impute.set,
     file="s2-model-perf-tables-post.RData")
```
